...
export default function SOPsContainer() {
  ...
  return (
    <div className="w-full min-h-screen bg-white text-[#37352f] font-sans">
      {!selectedSOP ? (
        <SOPsList sops={sopsData} onSelect={handleSelectSOP} />
      ) : (
        <SOPDetail sop={selectedSOP} onBack={handleBack} onSelect={handleSelectSOP} sops={sopsData} />
      )}
    </div>
  );
}
