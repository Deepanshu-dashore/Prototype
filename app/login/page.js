...
export default function LoginPage() {
  ...
  // Industries data with icons, names, and soft background colors (lighter bg, darker icons)
  const industries = [
  ...
  // Create chip-style cards for all industries (like the reference image)
  const industryCards = industries.map((industry) => ({
    node: (
      <div className="flex items-center gap-2.5 px-4 py-2.5 my-2 bg-white backdrop-blur-sm rounded-lg hover:bg-white transition-colors border-2 border-neutral-200 box-border">
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Store user data in cookies (token is already set by server in httpOnly cookie)
  ...
  return (
    <div className="min-h-screen flex bg-white">
      {/* Left Side - Industries Carousel with Multiple Strips */}
      <div className="hidden lg:flex lg:w-1/2 bg-neutral-100 relative overflow-hidden">
        <div className="absolute inset-0">
          {/* Rotated Industries Carousel at -45 degrees - Multiple Strips with All Industries */}
          <div className="absolute inset-0 transform -rotate-45 origin-center scale-[1.5]">
            <div className="w-full h-full flex flex-col justify-center gap-3">
              {/* Create 4-5 strips for better performance */}
              {[0, 1, 2, 3, 4].map((stripIndex) => (
                <div key={stripIndex} className="w-full overflow-hidden">
                  <LogoLoop
                    logos={industryCards}
                    speed={20 + (stripIndex * 3)} // Different speeds for each strip
                    direction={stripIndex % 2 === 0 ? 'left' : 'right'} // Alternate directions
                    gap={8}
                    showCard={false}
                    logoHeight={48}
                  />
      ...
      {/* Mobile Industries Display - Multiple Strips */}
      <div className="lg:hidden absolute bottom-0 left-0 right-0 h-64 bg-neutral-100/80 backdrop-blur-sm border-t border-neutral-300 overflow-hidden">
        <div className="absolute inset-0 transform -rotate-45 origin-center scale-[1.2]">
          <div className="w-full h-full flex flex-col justify-center gap-2">
            {/* Create 4 strips for better performance */}
