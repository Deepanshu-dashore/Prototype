...
export async function PATCH(request, { params }) {
  ...
  try {
    ...
    if (type === "po") {
      ...
      if (!result) {
        return ApiResponse(500, null, "Failed to upload PO to Cloudinary");
      }
      order.poLink = result;
      await order.save();
    ...
    if (type === "invoice") {
      ...
      if (!result) {
        return ApiResponse(500, null, "Failed to upload invoice to Cloudinary");
      }
    ...
    return ApiResponse(400, null, "Invalid type parameter");
  } catch (error) {
